...
class OtcController extends ApibaseController{
    ...
    public function actionMarketList(){
      ...
      //获取市场详情
  public function actionMarketDetail(){
    ...
    $market = (new \yii\db\Query())
      ->select('a.id,a.side,a.uid,a.coin_name,a.min_num,a.max_num,a.price_usd,card_enable,alipay_enable,wechat_enable,b.nickname,b.head_portrait,a.order_count,a.deal_count,a.deal_amount,a.note')
      ->from("{$tablePrefix}otc_market AS a")
      ->leftJoin("{$tablePrefix}member AS b",'a.uid = b.id')
      ->where(['a.id'=>$market_id])
      ->one();
    if ($market) {
      $market['max_num'] = sprintf("%.2f", $market['max_num']);
      if ($market['order_count'] <= 0){
        $market['deal_rate'] = '0';
      }else{
        $market['deal_rate'] = $market['deal_count'] / $market['order_count'] * 100;
      }
      $market['deal_rate'] = sprintf("%.2f", $market['deal_rate']);
      $market['deal_rate'] = $market['deal_rate'] ?? '0.00';
      $market['head_portrait'] = parent::get_user_avatar_url($market['head_portrait']);
      $this->success_message($market,'_Success_');
    }else{
      $this->error_message('_No_Data_Query_');
    }
