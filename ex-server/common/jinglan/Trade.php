...
class Trade extends Jinglan
{
    ...
    public static function market_v3($uid=0)
    {
        ...
        $market = ExchangeCoins::find()->where(['enable' => 1])->andWhere(['in', 'stock', $coin_keys])->andWhere(['in', 'money', $coin_keys])->select('stock,money,limit_amount as min_amount,taker_fee,maker_fee,decimals')->orderBy('listorder DESC')->asArray()->all();
        if ($uid) {
...
// p($market_money);
        ...
        foreach ($market_money as $x){
            ...
            foreach ($market as $k => $y){
                if ($y['money'] == $x){
                    ...
                    //p($y['money'].'USDT']);die();
                    $rpc_ret = $rpc->do_rpc("market.last", [$y['money'].'USDT']);
                    if ($rpc_ret['code'] == 0) {
                        $temp2['exchange_rate_usd'] = round((float)$coins[$y['money']]['usd'], 5);
                    }else{
                        $temp2['exchange_rate_usd'] = round((float)$rpc_ret['data'], 5);
                    }

                    // 新增汇率字段
                    $temp2['exchange_rate_cny'] = round((float)$temp2['exchange_rate_usd']*Jinglan::usd_to_cny(), 5);
    ...
    public static function order($uid){
        ...
        if($member->verified_level < 2){
            //$ret = array('code'=>511,'message'=>"請先完成Level 2認證");
            //die(json_encode($ret));
        }
...
//        $maker_fee_rate = '0.000001';
        ...
        if($assets[$bank_symbol]['available'] >= ($need_available + $need_fee)){//交易所 有余额，够本次交易
            ...
            //此处改为直接扣银行表，防止多处余额叠加造成扣费失败
            //$bank_balance = Bank::getBalance($uid,$bank_symbol);
            $bank_last_balance = BalanceLog::find()->where(['member_id'=>$uid,'coin_symbol'=>$bank_symbol,'network'=>0])->orderBy('id desc')->one();   
        ...
        //判断下单用户手否是机器人,如果是手续费设置为0
        $robot_info = Robot::find()->where(['uid'=>$uid])->one();
        if(!empty($robot_info)){
          $taker_fee_rate = "0";
          $maker_fee_rate = "0";
        }
        ...
        if ($type == 2) {
            //市价单
            $rpc_method = 'order.put_market';
            $rpc_params = [intval($uid), $market, intval($side), $amount, $taker_fee_rate, $source];
        } else {
            //限价单
...
//var_dump($rpc_params);die();
        ...
        if($rpc_ret['code'] == 0){
            ...
            if(empty($robot_info)){
                ...
                Yii::$app->db->createCommand()->insert("{$tablePrefix}coins_order", [
                    'order_id' => $order_ret['id'],
                    'uid' => $order_ret['user'],
                    'market' => $order_ret['market'],
                    'amount' => 0,
                    'amount_all' => $amount,
                    'stock' => $markets[$market]["stock"],
                    'money' => $markets[$market]["money"],
                    'side' => $order_ret['side'],
                    'price' => $order_ret['price'],
                    'ctime' => $order_ret['ctime'],
                ])->execute();
            }
        }
