...
def get_depth(market):
    ...
    data_ask = data['asks']
    data_bid = data['bids']
    return len(data_ask),len(data_bid)

def execOrder(info,marketinfo):
    print '-------------' 
    market_name = marketinfo[int(info[3])]
    #if market_name != 'XRPUSDT':
        #return
    print 'market_name:' + str(market_name)
    depth_count = get_depth(market_name)
...
def lastUpdateTime(cur):
    ...
    if data:
        for info in data:
            thread.start_new_thread(execOrder,(info,marketinfo,))
            #time.sleep(0.1)
