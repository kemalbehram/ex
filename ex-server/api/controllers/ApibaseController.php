...
class ApibaseController extends ActiveController{
	...
	//检测token
	public function checkTokenold($access_token){
	...
	protected function getUserInfoById($id){
		...
		$where['id']=$id;
		$select = 'id,username,password_hash,nickname,email,head_portrait,verified_status,verified_level,is_google_check,two_step_open_status,mobile_phone,mobile_phone_status,status,otc_merchant,code,son_1_num,son_2_num,son_3_num,invite_rewards,invite_fee_rewards,total_invite_rewards,freeze_rewards,frozen_rewards,access_token,bank_status,info_status';
	...
	//更新用户level_id
	protected function update_user_level($uid){
		$this->check_empty($uid,'_Error_Parameters_');
		$where['id']=$uid;
		$select = 'id,verified_level,email,mobile_phone,verified_status,bank_status,info_status';
		$result = Member::find()->select($select)->where($where)->asArray()->one();
		//var_dump($result);
		if($result){
			if($result['verified_level']<3){
				$level_id = 0;
				if(!empty($result['mobile_phone'])&&!empty($result['email'])){
					if($result['info_status']==1){
						$level_id = 1;
					}
				}
				...
				//var_dump($level_id);
				if($result['verified_level'] <> $level_id ){
					$member = Member::findOne(['id'=>$uid]);
					$member->verified_level = $level_id;
					$member->save();
				}									
			}
		}	
