...
{
    public static function create_v2($uid=0,$type="")
    {
        ...
        if (!$coin) {
            Jinglan::error_message('該幣種類型不存在！');
        }
        if($coin['recharge_enable']!='1'){
            Jinglan::error_message('該幣種暫停充值');
        }
        // 判断是否已经生成该币种类型的地址
        ...
        if($rpc_ret['code'] == 0){
            ...
            if ($has_this_wallet) {
            	if(empty($has_this_wallet->addr)){
			        $memberWallets_up = MemberWallet::findOne($has_this_wallet->id);
			        $memberWallets_up->addr = $ret_address;
			        $memberWallets_up->save(false);	
