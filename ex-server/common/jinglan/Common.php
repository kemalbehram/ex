...
class Common extends Jinglan
{
    ...
    public static function send_mob_varcode($mobile_phone,$area_code = '86'){
        if ($area_code == '86') {
        ...
        $varcode_result = Varcode::find()->where(['mobile_phone'=>$area_code.$mobile_phone])->one();
        if($varcode_result){
            if(time() - $varcode_result->attributes['updated_at'] < $limit_time*60){
                Jinglan::error_message($limit_time.'每分鐘內限發壹次');
            }
        }
        ...
        // if ($mobile_phone == '18800001111') {
        //     $code = '123456';
        // }

        $sms = new SMS($mobile_phone, $code);
        if ($area_code == '86') {
            $send_result = $sms->send($area_code);
        }else{
            $send_result = $sms->send($area_code);
        }
        ...
        if($send_result['code'] == 0) {//发送成功
            if(empty($varcode_result)) {//表中无记录
                ...
                $varcode_model->mobile_phone = $area_code.$mobile_phone;
