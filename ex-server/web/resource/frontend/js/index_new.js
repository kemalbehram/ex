...
//     console.log(timestamp + '循环请求第'+ (number_of_requests++) +'次');
...
$ul.on('click', 'li', function(e) {
    ...
    if (index == 0) { $('.tradViewFav').css('display', 'block');
        $('.tradViewContent').css('display', 'none') } else { $('.tradViewContent').css('display', 'block');
        $('.tradViewFav').css('display', 'none') }
    coinIndex = index - 1;
...
function update_tradViewContent(ele, res, rat, index, e, mon, main, monhigh, monlow, monlast) {
    ...
    monlast = (ele.exchange_rate_cny * last) || '';
    monhigh = (ele.exchange_rate_cny * high) || '';
    monlow = (ele.exchange_rate_cny * low) || '';
    //          console.log('-----')
    //                  console.log(ele);
    //                  console.log(res);
    //                  console.log('------')
    ...
    $('.tradViewContent  .List_mark_new tbody tr').eq(index).children().eq(2).find('.rmb').html('/ NT$' + (monlast - 0).toFixed(2));
    ...
    $('.tradViewContent  .List_mark_new tbody tr').eq(index).children().eq(5).find('.rmb').html('/ NT$' + (monhigh - 0).toFixed(2));


    //24h最低
    $('.tradViewContent  .List_mark_new tbody tr').eq(index).children().eq(6).find('.min_pire').html(toNonExponential(low - 0));
    $('.tradViewContent  .List_mark_new tbody tr').eq(index).children().eq(6).find('.rmb').html('/ NT$' + (monlow - 0).toFixed(2));

    //24h成交量
    $('.tradViewContent  .List_mark_new tbody tr').eq(index).children().eq(7).find('.price').html((deal - 0).toFixed(2));
}
        ...
        $.each(r.data, function(index, re) {
            ...
            window.revieceData5 = function(m) {
                ...
                $('.tradViewFav .addChang').append('<tr trade="' + ele.stock + '" currency_trade="' + ele.money + '"_"' + ele.stock + '" class="currencyList_' + ele.money + '_' + ele.stock + '">' +
                    ...
                    '<span class="pire">' + toNonExponential(last - 0) + ' </span>' +
                    '</a>' +
                    '</td>' +
                    '<td width="8%"  style="text-align: right; class="change_rate">' +
                    '<a href="/trade/' + ele.money + '/' + ele.stock + '" style="color:' + (rat < 0 ? "rgb(234,0,112)" : "rgb(112, 168, 0);") + ';">' + toNonExponential(rat - 0) + '%</a>' +
                    '</td>' +
                    '<td width="100" style="display:none;"></td>' +
                    '<td width="20%"  style="text-align:center;" class="max_pire_new">' +
                    '<a href="/trade/' + ele.money + '/' + ele.stock + '">' +
                    '<span class="max_pire">' + toNonExponential(high - 0) + '</span>' +
                    '</a>' +
                    '</td>' +
                    '<td width="20%" style="text-align:center;" class="min_pire_new">' +
                    '<a href="/trade/' + ele.money + '/' + ele.stock + '">' +
                    '<span class="min_pire">' + toNonExponential(low - 0) + ' </span>' +
                    '</a>' +
                    '</td>' +
                    '<td class="mov">' +
...
function domEle(ele, res, rat, index, e, mon, main, monhigh, monlow, monlast) {
    ...
    return ('<tr trade="' + stock + '" currency_trade="' + money + '"_"' + stock + '" class="currencyList_' + money + '_' + stock + '">' +
        ...
        '<span class="pire" style="color:' + (rat < 0 ? "rgb(234,0,112)" : "rgb(112, 168, 0);") + ';">' + toNonExponential(last - 0) + '</span><span class="rmb" style="color:#aaa;">/ NT$' + (monlast - 0).toFixed(2) + ' </span>' +
        '</a>' +
        '</td>' +
        '<td width="8%" style="text-align: right;" class="change_rate">' +
        ...
        '<a href="/trade/' + ele.money + '/' + ele.stock + '">' +
        '<span class="max_pire">' + toNonExponential(high - 0) + '</span><span class="rmb" style="color:#aaa;">/ NT$ ' + (monhigh - 0).toFixed(2) + '</span>' +
        '</a>' +
        '</td>' +
        '<td width="20%" class="min_pire_new">' +
        '<a href="/trade/' + money + '/' + stock + '">' +
        '<span class="min_pire">' + low + '</span><span class="rmb" style="color:#aaa;">/ NT$' + (monlow - 0).toFixed(2) + ' </span>' +
        '</a>' +
        '</td>' +
        '<td class="mov">' +
        '<a href="/trade/' + money + '/' + stock + '"><span class="price">' + (deal - 0).toFixed(2) + '</span><span class="rmb">&nbsp' + money + '</span>' + '</a>' +
        '</td></tr>')
}
...
function isLogin(getToken) { //登录显示是否添加自选
    ...
    http.post(getToken, {}, function(res) {
        ...
        //分类名称
        $('.favorite_view').children('li:not(.FavLi)').remove();
        data.map(function(el, index) {
            $('.favorite_view').append('<li class="' + (index == coinIndex ? 'active' : '') + '" trade="' + el.main + '"><span class="trading"><i style="background:url(' + el.main_icon + ') no-repeat;background-size:100% 100%;width: 18px;height:18px;display: inline-block;margin-right: 6px;position: relative;top: 2px;"></i>' + el.main + '</span></li>')
        })
