...
class WithdrawCash extends Jinglan
{
    ...
    public static function turnOut($uid = 0)
    {
        ...
        if($member->verified_level < 1){
            //$ret = array('code'=>511,'message'=>"請先完成Level 1認證");
            //die(json_encode($ret));
        }
        ...
        $useremail = Member::find()->select('email,mobile_phone')->where("id = ".$uinfo['id'])->asArray()->one();
    ...
    public static function applyLog($uid=0)
    {
        ...
        foreach ($withdraw_apply as $key => $value) {
            $withdraw_apply[$key]['value_dec'] = $value['value_dec'].($value['withdraw_fee']?('(手續費'.$value['withdraw_fee'].($value['current']?('|礦工費'.$value['current']):'').')'):'');
    ...
    public static function financialLog($uid=0)
    {
        ...
        foreach ($balance_log as $key => $value) {
            if ($value['detial_type'] == 'exchange') {
                if ($value['type'] == 1) {
                    $balance_log[$key]['detial_type'] = '用戶交易所收入';
                }else{
                    $balance_log[$key]['detial_type'] = '用戶交易所支出';
                }
            }
            if ($value['detial_type'] == 'transfer') {
                if ($value['type'] == 1) {
                    $balance_log[$key]['detial_type'] = '合约资产划入';
                }else{
                    $balance_log[$key]['detial_type'] = '划出到合约资产';
                }
            }            
            if ($value['detial_type'] == 'chain') {
                if ($value['type'] == 1) {
                    ...
                    $balance_log[$key]['detial_type'] = '提現成功';
                }
            }
            if ($value['detial_type'] == 'system') {
                if ($value['type'] == 1) {
                    $balance_log[$key]['detial_type'] = '系統加款';
                }else{
                    $balance_log[$key]['detial_type'] = '系統扣款';
                }
            }
            if ($value['detial_type'] == 'withdraw') {
                if ($value['type'] == 1) {
                    $balance_log[$key]['detial_type'] = '提現';
                }else{
                    $balance_log[$key]['detial_type'] = '提現';
                }
            }
            if ($value['detial_type'] == 'withdraw_fee') {
                if ($value['type'] == 1) {
                    $balance_log[$key]['detial_type'] = '提現手續費';
                }else{
                    $balance_log[$key]['detial_type'] = '提現手續費';
                }
            }
            if ($value['detial_type'] == 'release_invite_reward') {
                $balance_log[$key]['detial_type'] = '邀請獎勵釋放';                
            }            
        }
