  ...
  function toNonExponential_coin(num,market_name) {
    var fixed_num = format_decimals(market_name);
    return num.toFixed(fixed_num);
  }
	$('body').css({
		'opacity':1
	})
...
//console.log(money);
//最新价
$('.tradViewContent  .List_mark_new tbody tr').eq(index).children().eq(2).find('.pire').html(toNonExponential_coin(last-0,ele.name));
...
//24h最高
$('.tradViewContent  .List_mark_new tbody tr').eq(index).children().eq(5).find('.max_pire').html(toNonExponential_coin(high-0,ele.name));
$('.tradViewContent  .List_mark_new tbody tr').eq(index).children().eq(5).find('.rmb').html('/ NT$' + (monhigh-0).toFixed(2));


//24h最低
$('.tradViewContent  .List_mark_new tbody tr').eq(index).children().eq(6).find('.min_pire').html(toNonExponential_coin(low-0,ele.name));
                		...
                		$.each(r.data,function(index,re){
                			 http.sendData({
		                        ...
		                        window.revieceData5 = function(m){
			                        ...
			                        '<span class="pire">' + toNonExponential_coin(last-0,ele.name) +' </span>'+
			                        '</a>' +
			                        '</td>' +
			                        ...
			                        '<a href="/trade/' + ele.money+'/'+ele.stock + '">' +
			                        '<span class="max_pire">' + toNonExponential_coin(high-0,ele.name) + '</span>' +
			                        '</a>' +
			                        '</td>' +
			                        '<td width="20%" style="text-align:center;" class="min_pire_new">' +
			                        '<a href="/trade/' + ele.money+'/'+ele.stock + '">' +
			                        '<span class="min_pire">' + toNonExponential_coin(low-0,ele.name) + ' </span>' +
			                        '</a>' +
			                        '</td>' +
			                        '<td class="mov">' +
...
//                  console.log('------')
                    ...
                    return('<tr trade="' + stock + '" currency_trade="' + money + '"_"' + stock + '" class="currencyList_' + money + '_' + stock + '">' +
                        ...
                        '<span class="pire" style="color:'+(rat < 0 ? "rgb(234,0,112)":"rgb(112, 168, 0);")+';">' + toNonExponential_coin(last-0,ele.name) + '</span><span class="rmb" style="color:#aaa;">/ NT$'+(monlast-0).toFixed(2)+' </span>' +
                        '</a>' +
                        '</td>' +
                        '<td width="8%" style="text-align: right;" class="change_rate">' +
                        ...
                        '<a href="/trade/' + ele.money+'/'+ele.stock + '">' +
                        '<span class="max_pire">' + toNonExponential_coin(high-0,ele.name) + '</span><span class="rmb" style="color:#aaa;">/ NT$ '+(monhigh-0).toFixed(2)+'</span>' +
                        '</a>' +
                        '</td>' +
