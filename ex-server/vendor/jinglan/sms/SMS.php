...
class SMS
{
 ...
 /**
    ...
    /*请求*/
    public function send($area_code = '86'){
        //获取系统短信配置
    ...
    /*请求*/
    public function send_linebi($area_code = '86'){
        $lxm_accesskey = '0976163553';
        $lxm_secretkey = 'qwe2019';
        if(empty($lxm_accesskey) || empty($lxm_secretkey)){
            $ret = ['code'=>1,'message'=>'server busy'];
        }else{
            $host='http://api.message.net.tw/send.php?id='.$lxm_accesskey.'&password='.$lxm_secretkey.'&tel='.$area_code.$this->mobile_phone.'&msg=Your%20verification%20code%20is%20'.$this->code;
            //var_dump($host);die();
            ...
            if(strpos($ret,'ErrorCode=0') !== false){
                $ret = ['code'=>0,'message'=>$ret];
            }else{
                $ret = ['code'=>1,'message'=>$ret];
            }
        }
