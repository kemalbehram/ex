...
class ExchangeController extends MController
{
    ...
    public function actionMarket()
    {
        ...
        $rpc = new VesRPC();
        $map1 = 'market.user_deals';
        $map2 = [ (int)$user_id, $market, (int)$offset, (int)$limit];
    ...
    // 订单统计
    public function actionOrder_book()
    {
        // 获取筛选条件参数
        $request = Yii::$app->request;
        $market = $request->get('market','');
        ...
        $count_sql = "select count(id) as count from jl_coins_order where ".$where;
        $count=Yii::$app->db->createCommand($count_sql)->queryOne();
        ...
        $sql = "select sum(fee) as all_num from jl_coins_order where ".$where." and side=1";
        $data=Yii::$app->db->createCommand($sql)->queryOne();
        if (empty($data["all_num"])) {
            $all_sell_num = 0;
        }else{
            $all_sell_num = $data["all_num"];
        }
        $sql = "select sum(fee) as all_num from jl_coins_order where ".$where." and side=2";
        $data=Yii::$app->db->createCommand($sql)->queryOne();
        if (empty($data["all_num"])) {
            $all_buy_num = 0;
        }else{
            $all_buy_num = $data["all_num"];
        }

        $sql = "select * from jl_coins_order where ".$where." limit ".$offset.",".$limit.";";
        ...
        return $this->render('order-book2',$result);
    }
...
//                $withdraw_apply->updated_at    = time();
    ...
    // 提现审核【手工通过】
    public function actionApplyYes2()
    {
        $request = Yii::$app->request;
        $id      = $request->get('id');
        if (empty($id)) {
        ...
        if($withdraw_apply->type == 1){
            ...
            $withdraw_apply->status        = 6; //操作处理中
            $withdraw_apply->save();
            $result = $this->turnOutCC_v2($withdraw_apply,1);
            if ($result['code'] == 0) {//操作过程失败，重置为待审核
                $withdraw_apply->status        = 1;
//                $withdraw_apply->error_message = $result['msg'];
//                $withdraw_apply->updated_at    = time();
            ...
            if ($result['data']['tx_hash'] == 'withdraw_to_internal_member'){
                ...
                $update_apply_status = 2; //$update_apply_status = 5;  手工提币直接改成2 成功
            }
            $withdraw_apply->pay_user_type = $result['data']['pay_user_type'];// 支付人id
            $withdraw_apply->pay_from    = $result['data']['pay_addr'];// 支付人地址
            $withdraw_apply->status      = $update_apply_status;
        ...
        if ($balance_all){
            ...
            if ((float)$balance['exchange_available'] > 0){
                ...
                try{
                    ...
                    $bank_last_balance = BalanceLog::find()->where(['member_id'=>$uid,'coin_symbol'=>$balance['name'],'network'=>0])->orderBy('id desc')->one();   
                    if (empty($bank_last_balance)){
                        $bank_balance = 0;
                    }else{
                        $bank_balance = $bank_last_balance->balance;
                    }
            ...
            $bank_last_balance = BalanceLog::find()->where(['member_id'=>$uid,'coin_symbol'=>$coin_name,'network'=>0])->orderBy('id desc')->one();   
            if (empty($bank_last_balance)){
                $bank_balance = 0;
            }else{
                $bank_balance = $bank_last_balance->balance;
    ...
    // $manual = 0 自动  $manual = 1 手工线下提币
    protected function turnOutCC_v2($data,$manual=0){
        ...
        if ((float)$user_balance['exchange_available'] > 0){
            ...
            try {
                ...
                $bank_last_balance = BalanceLog::find()->where(['member_id'=>$uinfo['id'],'coin_symbol'=>$coin_symbol,'network'=>0])->orderBy('id desc')->one();   
                if (empty($bank_last_balance)){
                    $bank_balance = 0;
                }else{
                    $bank_balance = $bank_last_balance->balance;
        ...
        if (!empty($find_user_bank_addr) && $find_user_bank_addr['coin_symbol'] == "_{$coin_symbol}_"){//找到内部账号
            ...
            $bank_last_balance = BalanceLog::find()->where(['member_id'=>$find_user_bank_uid,'coin_symbol'=>$coin_symbol,'network'=>0])->orderBy('id desc')->one();   
            if (empty($bank_last_balance)){
                $bank_balance2 = 0;
            }else{
                $bank_balance2 = $bank_last_balance->balance;
            }
        ...
        if($manual==1){//手工提币 直接成功
            ...
            if($find_transaction){
                ...
                $find_transaction->tx_status = 'success';
