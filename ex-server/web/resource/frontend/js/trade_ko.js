...
window.openSend = function(r){
	...
	$('#trustbtnout').empty().append("매도&nbsp;"+stock)
	$('#trustbtnin').empty().append("매수&nbsp;"+stock)
...
window.revieceData1 = function(res) {
   ...
   $('.shiftRmb1').empty().append('/NT$'+((mon*last-0).toFixed(4)))
	...
	$('.total_top_new').append('<div class="coin_coin">' +
		...
		'<p><span><font data-i18n="b_1">거래량</font>&nbsp;<em id="24h_count" style="font-style: initial;">' + ((deal*1).toFixed(2)) +(m1) + '</em>&nbsp;</span><span class="rmbPrice"><i class="'+(rat<0?"active":"")+'"></i><em id="rmbPrice" style="color:'+(rat<0?"#eb6153" : "#77BA77")+'">' + (rat + '%') + '</em></span></p>' +
		'<p><span><font data-i18n="b_2" color="#999">최고</font>&nbsp;<em id="24h_max" style="font-style: initial;">' + high + '</em></span><span><font data-i18n="b_3">최저</font>&nbsp;<em id="24h_min">' + low + '</em></span></p>' +
		'</div>'
	)
...
function get_market_info(market_url){
		...
		$('#favorite_view').empty().append('<li trade="Fav" class="FavLi"><span class="collection"><font data-i18n="a_optional">선택</font></span></li>')
...
//					}
					...
					//console.log(indexArr);
						$('#sellCoin').append(
							'<tr height="20">' +
							'<td class="sell" width="50"><span class="title">매도' + (indexArr[index]) + '</span><span class="depth sell" style="width:' + (380.00 * rat) + 'px;"></span></td>' +
             ...
             if(res.result) {
				...
				records.map(function(res, index) {
					...
					$('#mycointrustlist_new').append('<tr height="30">' +
						'<td class="header" width="7%" style="font-size:15px;text-align:center;color:'+(res.side==1?"#e55600;":(res.side==2?"#090;":""))+'">'+(res.side==1?"<span class='spanStyle' style='font-size:20px ;'></span>매도":(res.side==2?"<span  class='spanStyle buyStyle' style='font-size:20px ;'></span>买":""))+'</td>' +
             ...
             if(r.result) {
				...
				$.each(data,function(index, r) {					
					...
					$('#myHistoryList_new').append('<tr height="30">' +
						...
						'<td class="header" width="7%" style="font-size:15px;text-align:left;color:'+(r.side==1?"#e62512;":(r.side==2?"#090;":""))+'">'+(r.side==1?"<span class='spanStyle' style='font-size:20px ;'></span>매도":(r.side==2?"<span  class='spanStyle buyStyle' style='font-size:20px ;'></span>买":""))+'</td>' +						
