...
class GatheringController extends ApibaseController{
    ...
    public function actionAddProceed(){
        ...
        $this->check_exist($member_id, $account, $proceeds_type);
     ...
     * 判断账号是否添加过
     */
    private function check_exist($member_id, $account, $proceeds_type){
        $exist = MemberProceeds::find()->where(['member_id' => $member_id, 'account' => $account, 'proceeds_type' => $proceeds_type, 'is_delete' => 0])->one();
        if(!empty($exist)){
            $this->error_message('_Have_Add__Account_Num_');
        }
    }
